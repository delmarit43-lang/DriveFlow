...
import { CalendarDays, Check, MapPin, Phone, Printer, Trash2, X } from "lucide-react";
...
export default function BookingDetailPage() {
  ...
  const { bookings, customers, vehicles, setBookingStatus, deleteBooking } = useFleet();
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-6">
      ...
      <ConfirmDialog
        ...
        description={`${booking.id} will be permanently removed from the system.`}
        confirmLabel="Delete booking"
        destructive
        onConfirm={async () => {
          try {
            await deleteBooking(booking.id);
            toast({ title: "Booking deleted", description: `${booking.id} was removed.`, tone: "warning" });
            router.push("/bookings");
          } catch (err) {
            toast({
              title: "Delete failed",
              description: err instanceof Error ? err.message : "Could not delete booking.",
              tone: "error",
            });
          }
        }}
