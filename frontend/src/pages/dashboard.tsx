...
export default function DashboardPage() {
  ...
  const fleetMix = analytics?.fleetMix ?? [];
  const revenueChart = analytics?.revenueChart ?? [];
  const yearlyRevenueChart = analytics?.yearlyRevenueChart ?? [];
  ...
  const exportReport = () => {
    ...
    toast({
      title: t("dash.reportExported"),
      description: t("dash.reportExportedDesc", { range }),
    });
  };
    ...
    <div className="mx-auto max-w-[1400px] space-y-8">
      <PageHeader
        ...
        description={t("page.dashboard.desc", {
          vehicles: dashboardStats.totalVehicles,
          regions: dashboardStats.regions,
        })}
        actions={
          <>
            <Button variant="secondary" onClick={exportReport}>
              <Download /> {t("dash.exportReport")}
            </Button>
            <NewBookingButton />
          </>
        }
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        <StatCard
          ...
          icon={<Clock3 />}
          tone="red"
          hint={<span className="text-xs font-semibold text-red-500">{t("dash.actionRequired")}</span>}
        />
        <StatCard
          label={t("dash.inMaintenance")}
          value={String(dashboardStats.inMaintenance)}
          icon={<Wrench />}
          tone="amber"
          hint={<Badge variant="warning">{t("dash.overdueCount", { count: 3 })}</Badge>}
        />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0 pb-4">
            <div>
              <CardTitle>{t("dash.revenuePerformance")}</CardTitle>
              <CardDescription>{t("dash.revenueGrowth")}</CardDescription>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} barSize={range === "6M" ? 32 : 18}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [formatCurrency(v), t("dash.revenue")]}
                />
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card>
          <CardHeader>
            <CardTitle>{t("dash.topCustomers")}</CardTitle>
          </CardHeader>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle>{t("dash.upcomingReturns")}</CardTitle>
