...
export function VehicleForm({ vehicle, onDone }: { vehicle: Vehicle | null; onDone: () => void }) {
  ...
  const onSubmit = handleSubmit(async (raw) => {
    ...
    try {
      if (vehicle) {
        await updateVehicle(vehicle.id, { ...values, name, engine: vehicle.engine });
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid gap-3.5 sm:grid-cols-2">
        ...
        <Field label="Model" error={errors.model?.message}>
          <Input placeholder="e.g. Land Cruiser Prado" {...register("model")} />
        </Field>
        <Field label="Plate number" error={errors.plate?.message}>
          <Input placeholder="e.g. SL-HGE-4821" {...register("plate")} />
        </Field>
        ...
        <Field label="Home location" error={errors.location?.message}>
          <Input placeholder="e.g. Maansoor Business Hub, Hargeisa" {...register("location")} />
        </Field>
      </div>
